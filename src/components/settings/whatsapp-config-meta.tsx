...
export function WhatsAppConfigMeta() {
  ...
  async function handleSave() {
    ...
    if (!config && (!accessToken.trim() || !tokenEdited)) {
      toast.error('O Access Token é obrigatório para a configuração inicial');
      return;
    }
    ...
    try {
      ...
      if (data.registered === false && data.registration_error) {
        ...
        toast.success(
          'Credenciais salvas e verificadas. O registro de recebimento foi ignorado (sem PIN) — veja o status de registro abaixo.',
          { duration: 10000 },
        );
        setPin('');
      } else {
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      toast.error('Não foi possível contatar o endpoint de verificação.');
    } finally {
  ...
  async function handleReset() {
    if (!confirm('Isso vai apagar a configuração atual do WhatsApp para você inserir novamente. Continuar?')) {
      return;
    }
    ...
    try {
      ...
      toast.success('Configuração apagada. Você já pode inserir suas credenciais novamente.');
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
      setAccessToken('');
